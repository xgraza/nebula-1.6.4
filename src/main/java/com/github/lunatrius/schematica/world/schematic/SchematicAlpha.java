...
public class SchematicAlpha extends SchematicFormat
{
    ...
    public SchematicWorld readFromNBT(NBTTagCompound tagCompound)
    {
        ...
        NBTTagList tileEntitiesList = tagCompound.getTagList(TILE_ENTITIES, 10);
        ...
        for (int x = 0; x < world.getWidth(); x++)
        {
            for (int y = 0; y < world.getHeight(); y++)
            {
                for (int z = 0; z < world.getLength(); z++)
                {
                    ...
                    String name = Block.blockRegistry.getNameForObject(world.getBlockRaw(x, y, z));
        ...
        for (TileEntity tileEntity : world.getTileEntities())
        {
            ...
            } catch (Exception e)
            {
                ...
                if (--count > 0)
                {
                    ...
                    Reference.logger.error(String.format("Block %s[%s] with TileEntity %s failed to save! Replacing with bedrock...", block, block != null ? Block.blockRegistry.getNameForObject(block) : "?", tileEntity.getClass().getName()), e);
                }
                localBlocks[pos] = (byte) Block.getIdFromBlock(Blocks.bedrock);
                localMetadata[pos] = 0;
                extraBlocks[pos] = 0;
            }
        }
