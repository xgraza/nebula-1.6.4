...
public class RenderHelper {
	...
	public static void drawCuboidSurface(Vector3f zero, Vector3f size, int sides, float red, float green, float blue, float alpha) {
		float delta = Reference.config.propBlockDelta.getValue();
        vecZero.set(zero.x - delta, zero.y - delta, zero.z - delta);
		vecSize.set(size.x + delta, size.y + delta, size.z + delta);
