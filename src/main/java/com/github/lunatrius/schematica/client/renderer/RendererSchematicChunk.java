...
//		}
            ...
            for (z = minZ; z < maxZ; z++)
            {
                for (x = minX; x < maxX; x++)
                {
                    try
                    {
                        ...
                        if (!isAirBlock)
                        {
                            if (Reference.config.propHighlight.getValue() && renderPass == 2)
                            {
                                if (block == Blocks.air && Reference.config.propHighlightAir.getValue())
                                {
                                    zero.set(x, y, z);
                                    size.set(x + 1, y + 1, z + 1);
                                    if (Reference.config.propDrawQuads.getValue())
                                    {
                                        RenderHelper.drawCuboidSurface(zero, size, RenderHelper.QUAD_ALL, 0.75f, 0.0f, 0.75f, 0.25f);
                                    }
                                    if (Reference.config.propDrawLines.getValue())
                                    {
    ...
    public void renderTileEntities(int renderPass)
    {
        ...
        GL11.glColor4f(1.0f, 1.0f, 1.0f, Reference.config.propAlpha.getValue());
    ...
    {
        if (!Reference.config.propEnableAlpha.getValue())
        {
