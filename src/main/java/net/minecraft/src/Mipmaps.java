...
public class Mipmaps
{
    ...
    public void uploadMipmaps(int xOffset, int yOffset)
    {
        IntBuffer staticBuffer = TextureUtils.getStaticBuffer(this.width, this.height);
        ...
        for (int i = 0; i < this.mipmapDimensions.length; ++i)
        {
            ...
            IntBuffer buf = staticBuffer;

            if (this.mipmapBuffers == null)
            {
                staticBuffer.clear();
                staticBuffer.put(this.mipmapDatas[i]);
            }
            else
            {
                buf = this.mipmapBuffers[i];
            }
            ...
            int mipWidth = dim.width;
            int mipHeight = dim.height;
            GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, level, xMipOffset, yMipOffset, mipWidth, mipHeight, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, buf);
            xMipOffset /= 2;
            yMipOffset /= 2;
        }
    }
