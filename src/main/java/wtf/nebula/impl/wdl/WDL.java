...
import wtf.nebula.impl.wdl.gui.GuiWDLMultiworld;
import wtf.nebula.impl.wdl.gui.GuiWDLMultiworldSelect;
...
public class WDL
{
    ...
    public static INetworkManager nm = null;
    ...
    public static void onItemGuiClosed()
    {
        ...
        if (lastEntity != null)
        {
            ...
            else
            {
                ...
                chatMsg("Unsupported entity cannot be saved:" + lastEntity.getEntityString());
            }
            ...
            else
            {
                if (windowContainer instanceof ContainerChest && te instanceof TileEntityChest)
                {
                    if (windowContainer.inventorySlots.size() > 63)
                    {
                        ...
                        if ((var9 = wc.getBlockTileEntity(lastX, lastY, lastZ + 1)) instanceof TileEntityChest)
                        {
                            var13 = (TileEntityChest)te;
                            tec2 = (TileEntityChest)var9;
                            var12 = new ChunkPosition(lastX, lastY, lastZ + 1);
                        }
                        else if ((var9 = wc.getBlockTileEntity(lastX, lastY, lastZ - 1)) instanceof TileEntityChest)
                        {
                            var13 = (TileEntityChest)var9;
                            tec2 = (TileEntityChest)te;
                            var12 = new ChunkPosition(lastX, lastY, lastZ - 1);
                        }
                        else if ((var9 = wc.getBlockTileEntity(lastX + 1, lastY, lastZ)) instanceof TileEntityChest)
                        {
                            var13 = (TileEntityChest)te;
                            tec2 = (TileEntityChest)var9;
                        ...
                        {
                            if (!((var9 = wc.getBlockTileEntity(lastX - 1, lastY, lastZ)) instanceof TileEntityChest))
                            {
    ...
    public static void savePlayer(NBTTagCompound playerNBT)
    {
        ...
        try
        {
            ...
            File playerFile = new File(e, tp.username + ".dat.tmp");
            File playerFileOld = new File(e, tp.username + ".dat");
    ...
    public static void saveChunks()
    {
        ...
        LongHashMapEntry[] hashArray = (LongHashMapEntry[])((LongHashMapEntry[])stealAndGetField(chunkMapping, LongHashMapEntry[].class));
        WDLSaveProgressReporter progressReporter = new WDLSaveProgressReporter();
        progressReporter.start();
        LongHashMapEntry[] arr$ = hashArray;
        int len$ = hashArray.length;

        for (int i$ = 0; i$ < len$; ++i$)
        {
            for (LongHashMapEntry lhme = arr$[i$]; lhme != null; lhme = lhme.nextEntry)
            {
    ...
    public static void applyOverridesToPlayer(NBTTagCompound playerNBT)
    {
        ...
        if (playerPos1.equals("xyz"))
        {
            ...
            ((NBTTagDouble)pos.tagAt(0)).data = (double)x + 0.5D;
            ((NBTTagDouble)pos.tagAt(1)).data = (double)y + 0.621D;
            ((NBTTagDouble)pos.tagAt(2)).data = (double)z + 0.5D;
    ...
    public static String getServerName()
    {
        try
        {
            ...
            {
                return mc.currentServerData.serverName;
            }
